...
/*initializing the variable*/
        ...
        var inputText = document.getElementById('excelRows').value;
        ...
        /*split at tab works*/
        var cleanText = inputText.replace(/\r\n|\r|\n/g, '~~~').split('~~~');


        var arrayArticles = [];
    var arrCount = 0;

        var artLength =	cleanText.length;
        for(var x = 0; x < artLength; x++ ){

            var artLines = cleanText[x];
            var splitLines = artLines.replace(/\t/g,'~~').split('~~');
            var splitLnLength = splitLines.length;

            for(var y=0; y < splitLnLength; y++ ){

                arrayArticles[arrCount] = splitLines[y];
    arrCount++;
